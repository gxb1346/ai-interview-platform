...
public class VoiceInterviewProperties {
    ...
    /**
     * 用户停止说话后自动提交的静默等待时间（毫秒）。
     * 设置为 0 或负数表示禁用自动提交，仅由前端手动 submit。
     */
    private int userUtteranceDebounceMs = 0;

    /**
     * LLM 流式调用超时时间（秒），默认 60 秒。
     * 超过此时间未收到完整回复则返回错误提示。
     */
    private int llmTimeoutSeconds = 60;

    /**
     * 是否启用四阶段流程（自我介绍 → 技术考察 → 项目深挖 → 反问环节）
     * 开启后遵循结构化面试流程，类似旧版模拟面试体验
     */
    private boolean enableFourPhaseFlow = true;
    /**
     * ASR 模型选择：qwen3-asr-flash-realtime（推荐，准确率更高） 或 paraformer-realtime-v2（兼容旧版）
     */
